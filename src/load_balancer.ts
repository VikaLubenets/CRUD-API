...
if (cluster.isPrimary) {
  ...
  const server = http.createServer((req, res) => {
    ...
    redirectURL.pathname = req.url || "";
    redirectURL.port = String(PORT + (currentIndex++ % numCPUs));
    res.writeHead(307, { Location: redirectURL.href });
    res.end();
  });
  ...
  if (workerId) {
    ...
    workerServer.listen(port, () => {
      console.log(
        `Worker with id ${workerId} and pid ${process.pid} started on port ${port}`,
      );
    });

    workerServer.on("request", (req) => {
      console.log(`Worker:${port} received request: ${req.method}:${req.url}`);
    });
  }
}
